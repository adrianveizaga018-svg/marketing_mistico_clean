...
const Footer = () => {
  ...
  return (
    <footer className="bg-black border-t border-[#c9a961]/20 pt-20 pb-10">
      ...
      {/* WhatsApp Floating Button - Official Icon 3D */}
      <div className="fixed bottom-8 right-8 z-50 animate-float-3d">
        <a 
          href={`https://wa.me/${footerData.whatsapp.replace(/\s/g, '')}`}
          target="_blank"
          rel="noopener noreferrer"
          onClick={() => metaPixel.trackWhatsAppClick()}
          className="w-16 h-16 bg-[#25D366] rounded-full flex items-center justify-center shadow-[0_4px_20px_rgba(37,211,102,0.4)] hover:scale-110 transition-transform group overflow-visible relative"
        >
          {/* Official WhatsApp SVG White */}
          <svg viewBox="0 0 24 24" className="w-9 h-9 fill-white drop-shadow-md" xmlns="http://www.w3.org/2000/svg">
          ...
          <span className="absolute -top-12 right-0 bg-[#1a1f2e] text-white px-4 py-2 rounded-xl text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity border border-[#c9a961]/30 shadow-xl font-medium">
            ¡Chatea con nosotros!
          </span>
        </a>
      </div>
